...
class NdnDirective(Directive):
    ...
    def run(self):
        self.assert_has_content()
        ndn_node = ndn(rawsource='\n'.join(self.content), **self.options)
        self.state.nested_parse(self.content, self.content_offset, ndn_node)
        self.add_name(ndn_node)
        return [ndn_node]

class ndn(nodes.Element):
    pass

def ndn_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ndn_node = ndn()
    ndn_node.children.append(nodes.Text(text))
    return [ndn_node], []

def visit_ndn_html(self, node):
    self.body.append('<ndn-name>')

def depart_ndn_html(self, node):
    self.body.append('</ndn-name>')

def visit_ndn_latex(self, node):
    self.body.append('\\ndnName{')

def depart_ndn_latex(self, node):
    self.body.append('}')
...
def setup(app):
    ...
    app.add_directive('ndn', NdnDirective)
    app.add_role('ndn', ndn_role)
    app.add_node(ndn,
            html = (visit_ndn_html, depart_ndn_html),
            latex = (visit_ndn_latex, depart_ndn_latex)
            )
